...
$(document).ready(function() {
    ...
    $('#list').DataTable({
	'responsive' : true, 'order' : [ [ 1, 'asc' ] ], 'paging' : true, 'sPaginationType' : 'full_numbers', 'searching' : true, 'ordering' : true, 'processing' : true, 'serverSide' : true, 'ajax' : contextPath + 'admin/employees/table', 'language' : {
	    'url' : contextPath + 'scripts/pt-br.json'
	}, 'columns' : [ {
	    'data' : 'hash', 'bSortable' : false, 'bSearchable' : false, 'render' : function(data, type, row) {
		if (type === 'display') {
		    return '<a href="' + contextPath + 'admin/employees/' + row.hash + '"><i class="fa fa-pencil-square-o text-success" aria-hidden="true"></i></a>';
		}
		return data;
	    },
	}, {
	    ...
	    'data' : 'selectionType', 'bSortable' : true, 'render' : function(data, type, row) {
		return data;
	    },
	}, {
	    'data' : 'allocation', 'bSortable' : true, 'render' : function(data, type, row) {
		return data;
	    },
	}, {
