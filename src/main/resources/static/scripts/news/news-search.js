...
$(document).ready(function() {
    ...
    $('#list').DataTable({
	'responsive' : true, 'order' : [ [ 1, 'asc' ] ], 'paging' : true, 'sPaginationType' : 'full_numbers', 'searching' : true, 'ordering' : true, 'processing' : true, 'serverSide' : true, 'ajax' : contextPath + 'admin/news/table', 'language' : {
	    'url' : contextPath + 'scripts/pt-br.json'
	}, 'columns' : [ {
	    'data' : 'hash', 'bSortable' : false, 'bSearchable' : false, 'render' : function(data, type, row) {
		if (type === 'display') {
		    return '<a href="' + contextPath + 'admin/news/' + row.hash + '"><i class="fa fa-pencil-square-o text-success" aria-hidden="true"></i></a>';
		}
		return data;
	    },
	}, {
	    ...
	    'data' : 'highlight', 'bSortable' : true, 'render' : function(data, type, row) {
		if (type === 'display') {
	    ...
	    'data' : 'date', 'bSortable' : true, 'render' : function(data, type, row) {
		return data;
	    },
	}, {
